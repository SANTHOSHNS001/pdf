...
var PDFAnnotate = function (container_id, url, options = {}) {
  ...
  this.fabricClickHandler = function (event, fabricObj) {
    ...
    if (s.length>0  ){
      ...
      name=inst.textBoxText
    }
    
    debugger;
    if (inst.active_tool == 2) {
      toolObj = new fabric.IText(name, {
...
PDFAnnotate.prototype.enableAddText = function (name) {
  console.log(name)
  var inst = this;
  alert("hi",inst)
...
PDFAnnotate.prototype.enableSquare = function () {
  var inst = this;
  var fabricObj = inst.fabricObjects[inst.active_canvas];
  inst.active_tool = 5; // Assuming 5 is the identifier for the circle tool
  if (inst.fabricObjects.length > 0) {
    $.each(inst.fabricObjects, function (index, fabricObj) {
      fabricObj.isDrawingMode = false;
    });
...
PDFAnnotate.prototype.enableCircle = function () {
  var inst = this;
  var fabricObj = inst.fabricObjects[inst.active_canvas];
  inst.active_tool = 6; // Assuming 6 is the identifier for the circle tool
  if (inst.fabricObjects.length > 0) {
    $.each(inst.fabricObjects, function (index, fabricObj) {
      fabricObj.isDrawingMode = false;
    });
